...
docker-compose kill ipfs0 ipfs1 ipfs2 ipfs3 cluster0 cluster1 cluster2 cluster3 web ganache
yes | docker-compose rm ipfs0 ipfs1 ipfs2 ipfs3 cluster0 cluster1 cluster2 cluster3 web ganache
rm -rf compose/ipfs0
rm -rf compose/ipfs1
rm -rf compose/ipfs2
rm -rf compose/ipfs3
...
docker exec ipfs0 ipfs config --json API.HTTPHeaders.Access-Control-Allow-Headers '["X-Requested-With","Range"]'
